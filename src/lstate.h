...
// 分代模式1——部分执行模式
// 若当前分配的内存总量仍在某个预设置的范围内，则算法采用部分执行模式。如字面意思，部分执行模式不需要处理所有的对象，
// 所以性能开销较低，若使用分代式算法，则应该尽量保持在大部分情况下，算法都处于该部分执行模式，
// 因为只有在这种执行模式下算法才能真正有效节省性能开销，想比于增量式算法才具有优势，部分执行模式简要步骤如下：
// 1）标记：只遍历处理所有老一代对象，若老一代对象已标记（黑色状态），则他们引用的年轻一代对象会被进行标记染色，
// 老一代对象本身保持黑色不变。年轻一代若没有被老一代对象引用，则不参与标记流程。
// 2）原子阶段：若上述被老一代对象引用的年轻对象为集合数据类型，则在此原子阶段会对它引用的子结点递归地进行标记，
// 把集合中所有的元素都标记上。另外，与增量式算法相同，原子阶段也会处理屏障与弱引用关系表。
// 3）清除：标记完成后，只遍历处理年轻一代对象，把未标记的年轻一代对象进行清除。
// 4）成长：与增量式算法的不同之处，清除阶段结束后算法仍有逻辑需要执行。在清除结束后，剩余下来的对象意味着在一轮GC中存活了，
// 此时会对他们的年龄进行增加：
// 1）老一代对象：仍然保持为老一代对象；
// 2）年轻一代对象：根据情况会在该轮或下一轮，也成长为老一代对象
// 分代模式2——全部执行模式
// 正常情况下，分代式算法不期望过多地进入全部执行模式，若当前分配的内存总量超过某个预设置的范围，代表此时内存使用过多，
// 需要释放更多的内存。此时分代式算法可以理解为在某种程度上失效了，就会采用这个消耗更大，处理对象更多，
// 但更能释放内存的全部执行模式。全部执行模式从代码层面也可以理解为增量式三色标记清除算法的全量式一次性完整执行的版本，
// 全部执行模式的简要步骤如下：
// 1）初始化：重置所有对象的颜色与年龄，把对象重置为白色与年轻一代。然后把当前GC算法类型临时设置为增量式算法，
// 以便于复用增量式标记清除算法的部分代码逻辑。
// 2）标记根结点：进入增量式算法第一阶段，标记以global_State全局状态机的某几个字段作为初始标记根结点。
// 3）原子阶段：遍历的根结点标记完成后，分代式算法这里会跳过增量式算法原本接下来该进入的可分步的标记传播阶段，
// 而是直接进入到再下一个阶段原子阶段。
// 原子阶段会对剩余的修改了未重新处理的所有引用可达的对象进行标记传播染色，相比于增量式算法经过了标记传播这一个阶段后再进入原子阶段，
// 分代式算法直接进入原子阶段，所以要标记的对象数量就会多很多。最后，在原子阶段中也会同样处理屏障与弱引用表的相关标记问题。
// 4）清除与全部变老：全部标记完成后，分代式算法也会执行清除工作，对未标记的对象进行清除，不同的地方是，清除结束后，
// 会把当前所有存活下来的对象年龄设置为老一代。
// 5）全部执行模式至此完成所有步骤，接下来会重新把当前GC算法类型恢复为分代式算法。
// 
// 算法退化
// 全部执行模式消耗较大，相比于增量式算法没有任何优势，一次性执行的任务较多，可能会导致程序卡顿，
// 我们不希望分代式算法经常进入全部执行模式。为了避免这种事情发生，我们就需要尽量把内存使用量控制在一个值以内。
// 1）保持：若全部执行模式流程结束后，内存使用量成功降低到一个水平以下，分代式算法会继续保持正常动作。
// 2）失效：若全部执行模式流程结束后，内存使用量没法降低到一个水平以下，代表随着后续的程序运行，
// 可能下次GC触发时分代式算法又不可避免地要再次使用全部执行模式。
// 在这种情况下，Lua会采用算法退化策略：
// 1）会先直接全量式、不打断地执行一次增量式三色标记清除算法的完整流程；
// 2）然后会临时保持每轮GC依旧使用不打断的增量式算法，直至某一轮GC清除的内存量符合某个阈值，才会重新恢复为正常的分代式算法逻辑。
