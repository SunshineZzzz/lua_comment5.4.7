...
l_sinline void moveresults (lua_State *L, StkId res, int nres, int wanted) {
  ...
  // 若函数的返回值多于调用者的期待值，则令返回值的个数减少到跟期待值一样就可以了，避免多余的堆栈数据赋值操作。
  if (nres > wanted)  /* extra results? */
    nres = wanted;  /* don't need them */
  for (i = 0; i < nres; i++)  /* move all results to correct place */
    setobjs2s(L, res + i, firstresult + i);
  // 若期待值数量大于返回值数量，则把多余的期待值赋值为nil。
 ...
 retry:
  switch (ttypetag(s2v(func))) {
    ...
    case LUA_VLCL: {  /* Lua function */
      ...
      int i;
      // 根据新调用的函数的参数个数对当前堆栈进行一个往下（栈底）的调整，复用当前空间，
      // 并用栈上的返回值对要调用的函数的参数进行一个设置。
      ...
      // 改变运行堆栈的顶部，使下一个执行的代码就是当前这个尾调用函数
...
*/
// 预处理函数(c/lua)会创建一个调用栈CallInfo，管理函数调用时的信息
 ...
 retry:
  switch (ttypetag(s2v(func))) {
    ...
    case LUA_VLCF:  /* light C function */
      ...
      return NULL;
    case LUA_VLCL: {  /* Lua function */
      // 当我们告诉虚拟机执行器要开始准备调用一个Lua函数的时候，
      // 虚拟机会把这个Lua函数对应的Lua闭包放到栈顶位置，
      // 然后这里准备调用信息，从栈顶的Lua闭包里面，
      // 提取出操作Lua运行栈所需要的全部信息（这些信息存储于Lua闭包的Proto中），
      // 然后放入到CallInfo中，每个CallInfo存储一个Lua闭包运行时栈相关实时动态信息
      CallInfo *ci;
      Proto *p = clLvalue(s2v(func))->p;
      // 实际传递的参数个数
